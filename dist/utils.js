...
const honeyProducedPerDayPerWorkerI = createSetting('Honey produced Per Day Per Worker', 0.0302, 0, 1); // Grams
const honeyConsumedPerDayPerWorkerI = createSetting('Honey Consumed Per Day Per worker', 0.011, 0, 1); // Grams
...
const workerMaxLifespanUpperBoundWinterI = createSetting('Worker Max Lifespan In Winter Upper Bound', 220, 0);
const percentOfWorkersWorkingI = createSetting('Percentage of all workers working', 0.9, 0, 1);
const honeyConsumedPerDayPerDroneI = createSetting('Honey consumed per day per drone', 0.0183, 0); // Grams
...
const honeyConsumedPerDayPerQueenI = createSetting('Honey consumed per day per queen', 0.01925, 0); // Grams
const queenEggLayingStartI = createSetting('The day the queen starts laying eggs', 60, 0, yearLength);
const queenEggLayingEndI = createSetting('The day the queen stops laying eggs', 240, 0, yearLength); // September 1
const maxHoneyCapacityI = createSetting('The maximum amount of honey the hive can hold', 140000, 0); // Grams
const maxNumberOfWorkersI = createSetting('The maximum number of workers a hive can hold', 80000 * percentageOfFertilizedEggsI(), 0);
const maxNumberOfDronesI = createSetting('The maximum number of drones a hive can hold', 80000 * (1 - percentageOfFertilizedEggsI()), 0);
const numberOfBeesEatenPerDayI = createSetting('The number of bees eater per day by predators', 100, 0);
...
const amountOfHoneyEatenIfHiveIsFoundI = createSetting('The amount of honey eaten if the hive is found', 50000, 0); // Grams
...
export { monthLength, yearLength, flowerBloomStartI, flowerBloomEndI, honeyProducedPerDayPerWorkerI, percentOfWorkersWorkingI, honeyConsumedPerDayPerWorkerI, honeyConsumedPerDayPerDroneI, droneMaxLifespanLowerBoundI, droneMaxLifespanUpperBoundI, percentageOfFertilizedEggsI, honeyConsumedPerDayPerQueenI, maxHoneyCapacityI, maxNumberOfDronesI, maxNumberOfWorkersI, numberOfBeesEatenPerDayI, percentChanceThatHiveIsFoundPerDayI, amountOfHoneyEatenIfHiveIsFoundI, };
