...
const simulationYearsI = createSetting('The number of years the simulation should be run for', 10, 0);
const updateFrequencyI = createSetting('How often the simulation recalculates the population', 1, 1, 5, 'By default, the simulation updates every day. High values will result in a faster but less accurate simulation');
...
const maxHoneyCapacityI = createSetting('The maximum amount of honey the hive can hold (grams)', 140000, 0); // Grams
...
const numberOfBeesEatenPerDayPerBirdI = createSetting('The number of bees eaten per bird per day', 2, 0);
